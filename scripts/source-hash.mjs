...
export function sourceHash(root = '.') {
    ...
    for (const file of files.filter((path) => tracked.has(resolve(path)) && !isNarrativeAsset(path)).sort()) {
        // Hash a root-relative label. Smoke calls sourceHash() with '.', while
        // playability passes an absolute gameRoot; hashing the raw path made
        // identical bytes produce different evidence hashes.
        hash.update(relative(resolve(root), resolve(file)).split(sep).join('/'))
        hash.update('\n')
