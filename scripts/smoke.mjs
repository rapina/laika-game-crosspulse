...
const REFRESH_CAPTURES = process.env.REFRESH_SMOKE_CAPTURES === '1'

async function saveCapture(page, path) {
    const screenshot = await page.screenshot()
    // The WebGL ripple is sampled between animation frames, so PNG bytes are
    // intentionally not deterministic even with a fixed rules seed. Routine
    // publish gates verify behavior without rewriting reviewed evidence; use
    // REFRESH_SMOKE_CAPTURES=1 when a source change requires new reference art.
    if (REFRESH_CAPTURES || !existsSync(path)) writeFileSync(path, screenshot)
}
...
async function main() {
    ...
    try {
        ...
        // Freeze before the first painted evidence settles. Waiting first lets
        // the prediction marker pass the center while the verification controls
        // are exercised, which mislabeled a late failure as verb-precision.
        await page.evaluate(() => window.dispatchEvent(new Event('blur')))
        await delay(600)

        mkdirSync('verification/captures', { recursive: true })
        const initial = await page.evaluate(() => globalThis.__gameState ?? null)
        const firstInputSilent = initial?.audioStarted === false
        await saveCapture(page, 'verification/captures/first-play.png')

        // DOM controls must not unlock gameplay audio. Toggle each and restore it.
        const localeBefore = initial?.locale
        await page.getByTestId('language').click()
        await delay(350)
        const localeChanged = await page.evaluate(() => globalThis.__gameState?.locale)
        await page.getByTestId('language').click()
        await delay(350)
        const localeRestored = await page.evaluate(() => globalThis.__gameState?.locale)
        const languageVerified = localeChanged !== localeBefore && localeRestored === localeBefore

        await page.getByTestId('mute').click()
        await delay(350)
        const mutedOn = await page.evaluate(() => globalThis.__gameState?.muted)
        await page.getByTestId('mute').click()
        await delay(350)
        const mutedOff = await page.evaluate(() => globalThis.__gameState?.muted)
        const muteVerified = mutedOn === true && mutedOff === false

        // Exercise the same browser lifecycle events used on real background/foreground.
        const pausedState = await page.evaluate(() => globalThis.__gameState ?? null)
        await page.evaluate(() => window.dispatchEvent(new Event('focus')))
        await delay(450)
        const resumedState = await page.evaluate(() => globalThis.__gameState ?? null)
        const pauseResumeVerified = pausedState?.paused === true
            && resumedState?.paused === false
            && resumedState?.tick > pausedState?.tick
        ...
        const tapAtOffset = async (offset, captureName) => {
            const before = await page.evaluate(() => globalThis.__gameState?.judged ?? 0)
            const deadline = Date.now() + 12000
            while (Date.now() < deadline) {
                const live = await page.evaluate(() => globalThis.__gameState ?? null)
                if (typeof live?.predictedCollisionY === 'number'
                    && live.predictedCollisionY >= live.bandCenter + offset) {
                    await tapCanvas()
                    break
                }
                await delay(30)
            ...
            while (Date.now() < deadline) {
                const judged = await page.evaluate(() => globalThis.__gameState?.judged ?? 0)
                if (judged > before) break
                await delay(30)
            }
            await delay(90)
            await saveCapture(page, `verification/captures/${captureName}.png`)
        }

        // The 30ms browser poll adds roughly 4px of visible travel in band 1.
        // Aim slightly above the center so the actual collision remains inside
        // the 21px precision window without reading hidden rule state.
        await tapAtOffset(-6, 'verb-precision')
        const audioUnlockedByGameplay = await page.evaluate(() => globalThis.__gameState?.audioStarted === true)
        await tapAtOffset(29, 'verb-success')
        await tapAtOffset(70, 'verb-failure')
        ...
        await saveCapture(page, 'verification/captures/game-over.png').catch(() => {})
        ...
        const summary = {
            ...
            restartVerified,
            firstInputSilent,
            audioUnlockedByGameplay,
            muteVerified,
            languageVerified,
            pauseResumeVerified,
            captures: ['first-play', 'verb-precision', 'verb-success', 'verb-failure', 'game-over'],
            consoleErrors,
            pageErrors,
        }
        ...
        if (!summary.firstInputSilent || !summary.audioUnlockedByGameplay) {
            console.error('FAIL: audio lifecycle did not remain silent until gameplay input')
            return 1
        }
        if (!summary.muteVerified || !summary.languageVerified || !summary.pauseResumeVerified) {
            console.error('FAIL: mute, language, or pause/resume behavior was not verified')
            return 1
        }
